...
export const getAllResponses = async () => {
    const {data} = await axios.get("/api/getResponses");
    if(!data) return
...
export const storeResponse = async (response: Response, ticket_userID: number) => {
    const {data} = await axios.post("/api/storeResponse", {content: response.content, ticket_id: response.ticket_id, ticket_userID: ticket_userID});
    if(!data) return
    responses.value = data;
}
...
export const updateResponse = async (response: Response) => {
    const {data} = await axios.patch("/api/updateResponse/" + response.id, {content: response.content});
    if(!data) return
    responses.value = data;
}