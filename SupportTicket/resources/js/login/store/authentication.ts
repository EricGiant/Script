...
export const authenticateUser = async (formData: Login) => {
    const data = await axios.post("/api/authenticateUser", {email: formData.email, password: formData.password}).catch((error) => error.response.status);
    return data;
}
...
export const forgotPassword = async (email: string) => {
    await axios.post("/api/sendResetPasswordEmail", {email: email});
...
export const updatePassword = async (password: string, token: string) => {
    await axios.patch("/api/updatePassword", {password: password, token: token});
